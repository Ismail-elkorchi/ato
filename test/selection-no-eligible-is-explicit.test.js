...
import { selectCycleQueueItem } from "../dist/core/cycle/select.js";
...
test("cycle selection with no eligible items returns structured error", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "ato-cycle-select-empty-"));
  const storeDir = ".ato";
  ...
  const items = [
    makeQueueItem({
      ...
      evidence: [],
      inputs: [],
      title: "Block-0001 empty evidence",
    }),
    makeQueueItem({
      id: "BL-0002",
      status: "done",
