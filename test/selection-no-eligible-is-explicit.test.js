...
test("selection due=false with no eligible items returns structured error", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "ato-eval-select-empty-"));
  const storeDir = ".ato";
  ...
  try {
    ...
    assert.equal(details.due, false);
